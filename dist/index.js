...
class SimpleCache {
    ...
    imageCache = new Map();
    CHAPTER_TTL = 10 * 60 * 1000; // 10 phút
    IMAGE_TTL = 30 * 60 * 1000; // 30 phút
    constructor() {
        // Dọn dẹp cache mỗi 10 phút
        setInterval(() => this.cleanup(), 10 * 60 * 1000);
    }
    setChapter(key, value) {
...
app.use("*", async (c, next) => {
    ...
    if (!userAgent)
        return c.text("User-Agent header is required.", 400);
    await next();
});
// CORS middleware
app.use("*", (c, next) => {
...
app.get("/ch/:id", async (c) => {
    ...
    try {
        ...
        if (!links) {
            ...
            links = fileNames.map((fileName) => `${baseUrl}/data/${hash}/${fileName}`);
            cache.setChapter(id, links);
        }
        const proxiedLinks = links.map((_, index) => `images/${id}/${index}`);
        return c.json({
            chapterID: id,
            images: proxiedLinks,
        }, 200);
...
app.all("*", async (c) => {
    try {
        ...
        if (targetPath === "/")
            return c.text("SuicaoDex API Proxy", 200);
